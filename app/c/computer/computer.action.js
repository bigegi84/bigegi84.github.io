const computerAction = {
  cat: (cmd) => {
    ...
    for (const key in computerStore.file) {
      if (key.search(file) != -1) {
        if (key.search("txt_") != -1) {
          console.log(key);
          computerStore.text.push(computerStore.file[key]);
        }
      }
    }
  },
  hostname: () => {
    computerStore.text.push(computerStore.hostname);
  },
  ls: () => {
    let res = "";
    for (const key in computerStore.file) {
      if (key.search("folder_") != -1) {
        res += `${key.replace("folder_", "")}\n`;
      }
      if (key.search("base64_") != -1) {
        res += `${key.replace("base64_", "")}\n`;
      }
      if (key.search("txt_") != -1) {
        res += `${key.replace("txt_", "")}\n`;
      }
    }
    computerStore.text.push(res);
  },
  pwd: () => {
    computerStore.text.push(computerStore.path);
  },
  answer: (text) => {
